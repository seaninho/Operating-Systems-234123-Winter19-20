...
module_param(program_name, charp,0000);
...
asmlinkage long our_sys_kill(int pid, int sig){//TODO: complete the function
   ...
   // struct task_struct* ptr = find_task_by_pid_(pid);
	struct task_struct* ptr;
	ptr= pid_task(find_vpid(pid),PIDTYPE_PID);
    if (ptr == NULL) {
    ...
    if (strncmp(program_name, ptr->comm, 16)==0 && sig == SIGKILL) {
        return -EPERM;
    }
    return original_syscall(pid, sig);
...
void plug_our_syscall(void){
    original_syscall=(typeof(our_sys_kill)*)sys_call_table[__NR_kill];//    original_syscall=(typeof(sys_kill) )sys_call_table[__NR_read];
